...
def get_number_of_matches(
    ...
    wine_matches_number = (
        search_result.json().get("explore_vintage", {}).get("records_matched")
    )
...
def get_price_ranges(
    min_price: int = 0, max_price: int = 10_000
    ...
    with open("price_ranges.csv", "r") as file:
        ...
        with requests.Session() as sess:
            for price_range in price_ranges:
                ...
                new_number_of_matches = get_number_of_matches(
                    sess, min_price, max_price
                )
                # If any price range exced the hardlimit, we need to re-compute all the price ranges
                if new_number_of_matches > HARD_LIMIT:
                    print(
                        f"Hard limit exceded or price range: {min_price} - {max_price} with {new_number_of_matches} matches found!"
                    )
                    return False
    return True
